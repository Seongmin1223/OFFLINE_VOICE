...
_EXTRACTION_PROMPT = """\
당신은 대화 기록에서 사용자 정보를 추출하는 분석 도구입니다.
역할 연기나 인사말 없이 아래 기준에 따라 JSON만 출력하세요.

추출 기준:
- "사용자:" 로 시작하는 발화에서만 정보 추출
- "AI:" 발화는 AI 측 발화이므로 모든 정보 제외
...
_SUMMARY_PROMPT = """\
당신은 대화 기록을 요약하는 분석 도구입니다.
역할 연기 없이 아래 기준에 따라 1~2문장으로만 출력하세요.

요약 기준:
- 사용자의 행동·관심사·새로 드러난 정보·중요한 사건 위주로 서술
- "사용자가 ~했다" / "~에 대한 대화가 있었다" 형식으로 객관 서술
- AI 측 발화는 화제 도출 단서로만 사용하고 요약 본문에는 포함하지 말 것
- AI 캐릭터 이름(예: 포비, 인형, 곰돌이 등)을 절대 출력에 포함하지 말 것
- 추측·확장·감상 금지
...
def _format_turns(turns: list[dict]) -> str:
    # NOTE: AI 측 라벨을 캐릭터 이름("포비")이 아닌 중립 "AI"로 유지.
    # 캐릭터명이 학습 컨텍스트에 박히면 추출/요약 LLM이 자기를 3인칭으로
    # 부르거나 페르소나에 빨려드는 환각이 생긴다.
    lines = []
    for t in turns:
        label = "사용자" if t["role"] == "user" else "AI"
